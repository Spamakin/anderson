...
\begin{defn}
  ...
  Each $s_i$ can be identified with the permutation swapping $i$ and $i + 1$.
...
\begin{prop}[{\cite[Proposition 1.3]{LLM+14}}]
...
\begin{prop}[{\cite{LM04, LM05}}]
  There is a bijection between the $(k + 1)$-cores $\kappa$ \quest{with $\abs{\kappa}_{k + 1} = m$} and the $k$-skew diagrams with \quest{something about $m$}.
\end{prop}
...
\begin{prop}[{\cite{LM04, LM05}}]
  There is a bijection between the $k$-bounded partitions $\lambda$ \quest{with $\abs{\lambda} = m$} and the $k$-skew diagrams with \quest{something about $m$}.
\end{prop}
...
\begin{ex}
  The word $w = s_1 s_3 s_2 s_0 s_3 s_2 s_1 s_0 \in \affS_4$ corresponds to the $(3 + 1)$-core $\kappa$ by the following sequence of steps:
  \begin{figure}[h]
    \begin{center}
      \includegraphics[width=0.375\textwidth]{images/affine_to_core.png}
      \caption{
  ...
  Below, we see that the residues of $\lambda = (3, 2, 2, 1)$ give the word $w$:
  \begin{figure}[h]
    \begin{center}
      \includegraphics[width=0.375\textwidth]{images/core_to_residue.png}
      \caption{
        Prior example of converting $\kappa$ to $\lambda$, with resides modulo $4$ given.
      }
    \end{center}
  \end{figure}
\end{ex}
