...
\end{lem}
This follows from the fact that branching programs are computed by determinants (\Cref{lem:abp_det}) and that the determinant has polynomial sized circuits (\Cref{prop:det_poly_ckt}).
...
There has been much focus around constant depth circuits
\begin{lem}
...
\begin{pf}
  \emph{Sketch:}
  Compute generic Gaussian elimination on an $n \times n$ matrix of variables.
  This gives a $\poly(n)$ sized circuit with division.
  Then eliminate divisions with \Cref{lem:div_elim} to get a $\poly(n)$ sized algebraic circuit computing $\det_n$.
\end{pf}
