...
\begin{frame}{Horner's Rule}
  ...
  \begin{itemize}
  \item This gives a circuit using exactly $d$ additions and $d$ multiplications
...
\begin{frame}{Border Waring Rank}
  ...
  This is an example where we cannot just set $\e = 0$.
\end{frame}
...
\begin{frame}{Trivial Statements can Arise}
  ...
  \begin{ex}
    \begin{itemize}
    \item Let $I \defeq \ideal{\perm_n, \det_n - \perm_n}$.
    \item It is conjectured that $\perm_n$, and thus $\det_n - \perm_n$, requires superpolynomial circuits.
    \item However, $\det_n = \perm_n + (\det_n - \perm_n) \in I$ is known to have a circuit of size $\poly(n)$ computing it.
    \end{itemize}
  \end{ex}
...
\begin{frame}{Dealing with Cancellations}
  \begin{itemize}
  \item We saw earlier that we have to be careful of cancellations when working with ideals.
  \item Working with the monomial basis is hard since determinants have exponentially many terms.
...
\begin{frame}
  Let $y_1, \ldots, y_n, z_1, \ldots, z_m, v$ be indeterminates.
  \begin{align*}
