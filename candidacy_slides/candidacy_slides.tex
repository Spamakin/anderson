...
\begin{frame}{Closure Results in Determinantal Ideals}    
    \begin{thrm}[{\cite[Theorem 1.1]{AF22}}]
        ...
        Then there is a depth-three $f$-oracle circuit of size $\bigO{n^2 m^2}$ that \emph{border computes} the $t \times t$ determinant for some $t = \poly\pqty{r^{1 / 3}}$.
    \end{thrm}
...
\begin{frame}{Closure Results in Determinantal Ideals}
    \begin{thrm}[{\cite[Theorem 1.5]{DG25}}]
        Let $f \in I^{\det}_{n, m, r}$ be a nonzero polynomial.
        Then there is a depth-three $f$-oracle circuit of size $\poly(n, m, \deg(f))$ that \emph{exactly computes} the $t \times t$ determinant for some $t = \poly\pqty{r^{1 / 3}}$.
    \end{thrm}
...
\begin{frame}{Dealing with Cancellations}
  ...
  \item Thus, we will work with a different basis for $I^{\det}_{n, m, r}$ which is described combinatorially with Young tabelau.
  \end{itemize}
\end{frame}
...
\begin{frame}{A Natural Generating Set for Determinantal Ideals}
  ...
  \begin{thrm}[{\cite[\S 8, Theorem 3]{DRS74},~\cite[Corollary 2.3]{dCEP80}}]
    ...
    \]
    where $A$ and $B$ are \emph{conjugate semistandard}: strictly increasing along rows, weakling increasing down columns.
  \end{thrm}
\end{frame}
...
\begin{frame}{Pfaffian Analogues}
    ...
    \begin{thrm}[{\cite[Theorem 1.6]{DG25}}]
        ...
        Then there is a depth-three $f$-oracle circuit of size $\poly(n, \deg(f))$ that \emph{exactly computes} the $t \times t$ Pfaffian for some $t = \poly\pqty{r^{1 / 3}}$.
    \end{thrm}
