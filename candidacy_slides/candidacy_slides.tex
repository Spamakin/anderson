...
\begin{frame}{Hard Polynomials Exist}
  ...
  \begin{sketch}
    \begin{itemize}
    \item Count the number of unlabeled circuits of size $s$.
    \item For a circuit of size $s$, labeling the edges gives a map $\Phi\colon \F^s \to \F^N$
    \item $\dim \im \Phi \leq s$, and there are only finitely many unlabeled circuits.
...
\begin{frame}{Determinant versus Permanent}
  \begin{conj}\label{conj:det_vs_perm}
    ...
    Then $m$ must be superpolynomial in $n$.
  \end{conj}
\end{frame}
...
\begin{frame}{Border Waring Rank}
  ...
  \begin{ex}[{\cite{DL25}}]
    ...
    In other words, $\Set{f(x, y) \in \C[x, y]_{\leq d} \mid \WR(f) = 2}$ is not Zariski-closed.
  \end{ex}
\end{frame}
...
\begin{frame}{Trivial Statements can Arise}
  \pause

  \begin{question}
      Suppose $0 \neq f \in \gen{g_1, \ldots, g_k}$.
...
\begin{frame}{A Natural Generating Set for Determinantal Ideals}
  ...
  \begin{thrm}[{\cite[\S 8, Theorem 3]{DRS74},~\cite[Corollary 2.3]{dCEP80}}]
    ...
    Furthermore, if $(A \mid B)$ has shape $\tau$ then $\tau \geq \sigma$, $c_{A,B} \in \mathbb{Z}$.
  \end{thrm}
\end{frame}
...
\begin{frame}{Proof Idealistical}
...
\begin{frame}{Partial Progress}
  \begin{thrm}
    ...
    Then there exists a partition $\sigma$ and a depth-three $f$-oracle circuit of size $\bigO{d^3 n^7}$ computing $(K_\sigma \mid K_\sigma)(X)$.
  \end{thrm}
...
\begin{frame}
    ...
    \begin{center}
        \item \quoteAuthorDate{If you don't know how to answer someone's question, you should complement them by saying it's a good question.}{Noga Alon}
    \end{center}
