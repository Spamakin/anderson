...
\begin{frame}{Linear Operators}
    \begin{defn}[Substitution operator~\cite{dCEP80}]
...
\begin{frame}{Where Does Isolation Arise?}
  ...
  \[
    g(X) = \sum_{k\in I} c_k \Lambda^{\overline{e}_k}\Xi^{\overline{f}_k}Y^{\overline{s}_k}Z^{\overline{t}_k}\cdot {\color{sigma@mainblue}(K_{\sigma_k}\mid K_{\sigma_k})(X)} \in \F[\Lambda, \Xi, Y, Z][X].
  \]
